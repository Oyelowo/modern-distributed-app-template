...
function extractCompile() {
	...
	// a localisation tool
	const messagePath = `${outDir}/for-translator/${LOCALE}.json`;
	...
	const compile = ({
		...
		return `pnpm formatjs compile '${messagePath}' --out-file ${outDir}/lang/${LOCALE}.json ${withAstFlag} \
     --out-file ${outDir}/compiled-lang/${LOCALE}.json && ${removeExtracted}`;
	};
...
export function updateRightLocale(
    ...
    */
	const rightObjectWithoutStaleKeys = R.pick(R.keys(leftObject), rightObject);
