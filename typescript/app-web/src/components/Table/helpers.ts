...
export type FilterConditionCompound<
	Operator extends string,
	Filter extends FilterValueType,
> = FilterConditionSimple<Operator, Filter> & {
	logical: OperatorLogical | null;
};

export type AddMeta = (meta: FilterMeta) => void;
export type FilterPropsOld<
	Operator extends string,
...
const columnFilterMetaSchemaValidator = z.object({
	itemRank: z.any(),
});

const columnFilterSchema: z.ZodType<{ itemRank?: RankingInfo }> =
	columnFilterMetaSchemaValidator;

export const fuzzySort: SortingFn<any> = (rowA, rowB, columnId) => {
	let dir = 0;
	const rowAColumn = columnFilterSchema.parse(rowA.columnFiltersMeta[columnId]);
	const rowBColumn = columnFilterSchema.parse(rowB.columnFiltersMeta[columnId]);

	// Only sort by rank if the column has ranking information
	if (rowAColumn.itemRank && rowBColumn.itemRank) {
		dir = compareItems(rowAColumn.itemRank, rowBColumn.itemRank);
	}
...
type ExtractRowProps<TRow extends RowValueType> = Extract<
	RowCustom,
	{ rowValue: TRow }
>;
...
export type FilterMultipleProps<TRow extends RowValueType> =
	FilterSingleProps<TRow> & LogicalProp;
export type RowFilterMultipleForm<TRow extends RowValueType> =
	FilterMultipleProps<TRow> & {
		key: string;
	};
