...
export const PrepareScalarPaths = ({
	ops,
	returns,
}: { returns: ReturnTypesType; ops: Operations }) => {
	const ibb = (
		k: string,
		originalKey: string,
		o: InputValueType | VType,
...
The input/output is a string in RFC3339 format. */
	...
	["Mutation"]: AliasType<{
		createUser?: [
			{ userInput: ValueTypes["UserInput"] | Variable<any, string> },
			ValueTypes["User"],
		];
		signUp?: [
		...
		signIn?: [
			{
				signInCredentials:
					| ValueTypes["SignInCredentials"]
					| Variable<any, string>;
			},
			ValueTypes["User"],
		];
		signOut?: ValueTypes["SignOutMessage"];
		createPost?: [
			{ post: ValueTypes["PostInput"] | Variable<any, string> },
			ValueTypes["Post"],
		];
		__typename?: boolean | `@${string}`;
	}>;
	...
	["Query"]: AliasType<{
		me?: ValueTypes["User"];
		user?: [
			{ id: ValueTypes["ObjectId"] | Variable<any, string> },
			ValueTypes["User"],
		];
		getUser?: [
			{ userBy: ValueTypes["UserBy"] | Variable<any, string> },
			ValueTypes["User"],
		];
...
The input/output is a string in RFC3339 format. */
	...
	["Mutation"]: AliasType<{
		createUser?: [
			{ userInput: ResolverInputTypes["UserInput"] },
		...
		signIn?: [
			{ signInCredentials: ResolverInputTypes["SignInCredentials"] },
			ResolverInputTypes["User"],
		];
		signOut?: ResolverInputTypes["SignOutMessage"];
		createPost?: [
			{ post: ResolverInputTypes["PostInput"] },
	...
	["Query"]: AliasType<{
		...
		getUser?: [
			{ userBy: ResolverInputTypes["UserBy"] },
			ResolverInputTypes["User"],
		];
...
Currently like this because of future developments */
		signUp: ModelTypes["User"];
		signIn: ModelTypes["User"];
		signOut: ModelTypes["SignOutMessage"];
...
Currently like this because of future developments */
		...
		signIn: GraphQLTypes["User"];
		signOut: GraphQLTypes["SignOutMessage"];
