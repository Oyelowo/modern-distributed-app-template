	...
	Object.entries(helmChartsInfo).forEach(([repoName, { repo, charts }]) => {
		...
		Object.values(charts).forEach((chartInfo) => {
			...
			missingCrdJsonSchemasLinks.forEach((schemaPath) => {
				...
				sh.exec(`curl  ${schemaPath} | pnpm json2ts  -o ${outPath} --format=false`);
			});
...
function syncHelmChartTypeDefs({ repoName, chart, version, helmChartsDir }: SyncHelmChartTypesProps) {
